...
\text{ and }\mathbf{\Lambda}_2=\begin{pmatrix}\lambda_1&0\\0&\lambda_2\end{pmatrix},$$ and computing $\mathbf{A}_2\mathbf{\Lambda}_2\mathbf{A}_2'$ (shown in Table 4). The most obvious difference between this reconstruction and the true correlation matrix is that the diagaonal values of the reconstruction are less than 1. This happens because not all of the variation in the individual variables occurs in these two dimensions.

Table 5 shows the difference, $\mathbf{R}-\mathbf{A}_2\mathbf{\Lambda}_2\mathbf{A}_2'$. Most of the off-diagonal entries are negative, implying that the variables are more strongly correlated when projected into the principal component space.
...
pander(cor(cc2015Rs_2) - pcs$rotation[,1:2] %*% diag(pcs$sdev[1:2]^2) %*% t(pcs$rotation[,1:2]),
       caption="Difference between original correlation matrix and reconstruction from the first two PCS.",
       split.table = Inf)
```
...
```
TODO
There is a sign difference for PC3 loadings between the two and I don't know why.

5. _What are the advantages in terms of interpretation and potential calculations from this re-scaling?_

Re-scaled PC loadings have also included information about the proportion of variance explained by each PC.  This information is immediately useful in many situations.  For example, 
