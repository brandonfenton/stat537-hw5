...
```
Table 6 shows the result of the calculation __t(pcs$sdev * t(pcs$rotation)__  using the results from __prcomp()__, while Table 7 shows the results from __principal()__.  Note that there is a sign difference in the loadings for PC3 between the two methods, but since the vectors are scalar multiples of each other they will fall along the same axis.  This is an artifact of the algorithm used to determine the eigenvectors, and will not affect any interpretation aside from changing the direction.


5. _What are the advantages in terms of interpretation and potential calculations from this re-scaling?_

Re-scaled PC loadings have also included information about the proportion of variance explained by each PC.  This information is immediately useful in many situations.  For example, biplots blah blah blah
