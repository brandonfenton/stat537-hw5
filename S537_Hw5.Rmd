...
pander(zapsmall(crossprod(pcs$rotation)),
       caption="Crossproduct of the rotation matrix")
```


2. _Use all the eigenvectors and eigenvalues to show that you can re-construct the entire correlation matrix._

```{r p2_a, echo=T, comment=NA, fig.pos="H", fig.align="center", cache=T}
pander(cor(cc2015Rs_2), caption="Correlation matrix of original data")
pander(pcs$rotation %*% diag(pcs$sdev^2) %*% t(pcs$rotation),
       caption="Reconstructed correlation matrix")
```
